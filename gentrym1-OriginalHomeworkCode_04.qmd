...
print("Confidence Intervals for Beta1 - Normal")
confint(m, level = .9)[2,]
print("Confidence Intervals for Beta1 - Log")
confint(log.m, level = .9)[2,]
```
...
```{r}
#| label: Confidence and Prediction Lines I
...
ints.pivot <- ints %>%
  pivot_longer(cols = c(fit, lwr, upr,
                        fit.1, lwr.1, upr.1),
               names_to = "type",
               values_to = "vals"
    )

log.ints <- data.frame(cbind(log.b, log.l, log.CI, log.PI))
...
```{r}
#| label: Confidence and Prediction Lines II
#| code-fold: true

p3 <- ggplot(data = ints.pivot, aes(x = b)) +
...
p4 <- ggplot(data = log.ints.pivot, aes(x = log.b)) +
